...
**۲.** در پنل هاست یک پایگاه داده‌ی خالی و یک کاربر بساز، و **کاربر را به پایگاه داده وصل کن** با همه‌ی دسترسی‌ها.
...
```
https://نشانی-سایتت/install.php
```

<div dir="rtl" align="right">

**۴.** فرم را پر کن — نام سایت، مشخصات پایگاه داده، و نام کاربری و رمز مدیر.

نصب‌کننده بقیه را خودش انجام می‌دهد:

- پیش‌نیازها را بررسی می‌کند و اگر چیزی کم بود می‌گوید کدام
- اتصال به پایگاه داده را همان‌جا آزمایش می‌کند و اگر نشد، دلیلش را به فارسی می‌گوید
- کلید رمزنگاری پیام‌های خصوصی را تصادفی می‌سازد
- فایل تنظیمات را می‌نویسد — ترجیحاً **بیرون از پوشه‌ی عمومی**
- هجده جدول را می‌سازد
- حساب مدیر را می‌سازد و واردت می‌کند

**۵.** بعد از نصب، فایل `install.php` را از سرور پاک کن.

</div>

<div dir="rtl" align="right">

> 🔐 **رمز پیش‌فرض وجود ندارد و عمداً هم نخواهد داشت.** کبوتر متن‌باز است؛ هر رمز پیش‌فرضی که در کد بنویسیم در همین مخزن برای همه پیداست و ربات‌ها نصب‌های تازه را با همان می‌گیرند. رمز خودت را در نصب‌کننده می‌سازی.

### نصب دستی

اگر ترجیح می‌دهی خودت فایل تنظیمات را بنویسی، `config.sample.php` را به `config.local.php` کپی کن و پرش کن. جدول‌ها همچنان خودکار ساخته می‌شوند. اولین حسابی که ثبت‌نام کند سازنده می‌شود.
